...
def weather(request):
    ...
    try:
        ...
        alert = []  # 알림을 리스트로 변경하여 여러 메시지를 추가할 수 있게 함
        today_min = daily[0]['temp']['min']
        today_max = daily[0]['temp']['max']

        # 비 예보가 있을 때 알림
        if 'rain' in current['weather'][0]['description']:
            alert.append("오늘은 비 예보가 있습니다. 우산 챙기는 것 잊지마세요.")
        # 맑은 날씨일 때 알림
        elif 'clear' in current['weather'][0]['description']:
            alert.append("오늘 날씨는 화창합니다. 나들이를 가는 건 어떨까요?")
        # 일교차가 10도 이상일 때 알림
        if today_max - today_min >= 10:
            alert.append("오늘은 일교차가 10도 이상입니다. 감기 조심하세요.")

        # 알림이 있다면 이를 하나의 문자열로 결합
        alert_message = " ".join(alert) if alert else None
        ...
        return render(request, 'weather.html', {
            ...
            'alert': alert_message,  # 결합된 알림 메시지 전달
        })
